...
if (cluster.isMaster) {
  ...
  /*     await mongoApi.connectMongoose();
    allSymbols = await getAvailableSymbolNames();
    allSymbols = ['ACA'];
    symbolsLeftToProcess = allSymbols.length;
    symbolChunks = chunkArray(allSymbols, SYMBOL_CHUNK_SIZE);
 */
  symbolChunks = [['SPY']];
  ...
  cluster.on('exit', async (worker, code, signal) => {
    ...
    if (symbolChunks.length > 0) {
      ...
      console.log(`symbolsLeftToProcess: ${symbolsLeftToProcess}`);
  ...
  process.on('message', (message) => {
    ...
    /*     (async () => {
      await mongoApi.connectMongoose();
      console.log('message from the master');
